                           ...
                           unsigned long count, void *data)
{
	...
	printk(KERN_DEBUG "Successfully allocated via vmalloc %u bytes (%u Kb, %u MB) now to location 0x%08x (will vfree..)\n", 
                           ...
                           unsigned long count, void *data)
{
	...
	printk(KERN_DEBUG "Successfully allocated via kmalloc %u bytes (%u Kb, %u MB) now to location 0x%08x (will kfree..)\n", 
			pstCtx->kvalloc, pstCtx->kvalloc/1024, pstCtx->kvalloc/(1024*1024), (u32)pstCtx->vm);
...
static int __init vmall_init_module(void)
{
	...
	if (!gpstCtx) {
		printk(KERN_DEBUG "%s: kmalloc 1 failed, aborting..\n", DRVNAME);
		status = -ENOMEM;
		goto out_k1_fail;
	}
	...
	if (!gpstCtx->kname) {
		printk(KERN_DEBUG "%s: kmalloc 2 failed, aborting..\n", DRVNAME);
		status = -ENOMEM;
		goto out_k2_fail;
	}
	...
	if (!gpstCtx->vname) {
		printk(KERN_DEBUG "%s: kmalloc 3 failed, aborting..\n", DRVNAME);
		status = -ENOMEM;
		goto out_k3_fail;
	}
	...
	if (!res) {
		printk(KERN_DEBUG "%s: Entry %s creation failure, aborting..\n", DRVNAME, gpstCtx->kname);
		status = -ENOMEM;
		goto out_p1_fail;
	}
