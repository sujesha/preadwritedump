 ...
 * We can/use for file output also? */
                 ...
                 struct tip_subbuf *ts)
{
    ...
    while (offset + sizeof(*t) <= ts->len) {
        ...
        if (offset + sizeof(*t) + elt_len > ts->len)
            break;

        offset += sizeof(*t) + elt_len;
        tip->events_processed++;
        tip->data_read += sizeof(*t) + elt_len;
        events++;
    }
...
static void get_and_write_events(void)
{
    ...
    while (!is_done()) {
        ...
        /* if no events in all of tips in all dips, then sleep 100 ms */
        if (!events)
            usleep(100000);
    }
    /*
